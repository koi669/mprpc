...
void RpcProvider::NotifyService(google::protobuf::Service *service)
{
    ...
    for (int i = 0; i < methodCnt; ++i)
    {
        ...
        const google::protobuf::MethodDescriptor *pmethodDes = pserviceDesc->method(i);
...
void RpcProvider::Run()
{
    ...
    // 绑定连接回调和消息读写回调方法 用muduo库分离了网络代码和业务代码
    ...
    //  session timeout 30s 心跳时间zkclient API提供心跳机制网络I/O线程1/3 timeout时间发送ping消息告诉znode节点依然存在
    ZkClient zkCli;
    zkCli.Start();
    ...
    for (auto &sp : m_serviceMap)
    {
        ...
        std::string service_path = "/" + sp.first; // 创建节点FriendServiceRpc节点/FriendServiceRpc永久性节点或者UserServiceRpc
        zkCli.Create(service_path.c_str(), nullptr, 0);
        for (auto &mp : sp.second.m_methodMap)
        {
            // service name/method name	/UserServiceRpc/Login存储当前这个rpc服务节点主机的ip和port
            std::string method_path = service_path + "/" + mp.first;
            char method_path_data[128] = {0};
            sprintf(method_path_data, "%s:%d", ip.c_str(), port); // ip : port
            // ZOO_EPHEMERAL表示znode是一个临时节点zkserver断开即销毁
...
void RpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr &conn, google::protobuf::Message *response)
{
    ...
    {
        // 序列化成功后，通过网络把rpc方法执行的结果发送会rpc的调用方法
